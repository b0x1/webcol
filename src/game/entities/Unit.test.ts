...
describe('Unit', () => {
  it('should initialize with correct values', () => {
    const unit = createUnit('unit-1', 'player-1', 'Test Unit', UnitType.COLONIST, 10, 10, 3);
  ...
  it('should have an empty cargo map upon initialization', () => {
    const unit = createUnit('unit-1', 'player-1', 'Test Unit', UnitType.SHIP, 0, 0, 5);
    expect(unit.cargo instanceof Map).toBe(true);
    expect(unit.cargo.size).toBe(0);
  });

  it('should be able to store and retrieve cargo', () => {
    const unit = createUnit('ship-1', 'player-1', 'Test Unit', UnitType.SHIP, 0, 0, 5);
